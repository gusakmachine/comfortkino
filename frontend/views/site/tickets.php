    ...
    <div class="popup__timetable">
        <div class="flex-wrapper">
            <?php for ($i = 0; $i < count($session['time']); $i++): ?>
                <button class="popup__timetable-link film__sessions-info" data-SH="#popup">
                    <span class="popup__timetable-time film__session-time session-time"><?= date('H:i', strtotime($session['time'][$sessionTimeIDX]['time'])); ?></span>
                    <span class="popup__timetable-price film__session-price session-price rub">от <?= $session['timePrices'][$sessionTimeIDX]['price'] ?></span>
                </button>
            <?php endfor; ?>
        </div>
    </div>
...
<!--                        </div>-->
                        <?php for($i = 0, $j = 1; $i < count($hall['placesSets']) - 1; $i++): ?>
                            <span class="place-row" style="left: 0; top: <?=  $hall['placesSets'][$i]['graphic_display']['top'] ?>px;"><?= $i + 1 ?></span>
                            <span class="place-row" style="left: 100%; top: <?=  $hall['placesSets'][$i]['graphic_display']['top'] ?>px;"><?= $i + 1 ?></span>
                            <?php for(; $i < count($hall['placesSets']) - 1 && $hall['placesSets'][$i]['row'] == $hall['placesSets'][$j]['row']; $i++): ?>
                                <button class="place scheme-menuplace"
                                        style="background: <?=  $hall['placesSets'][$i]['colors']['color'] ?> ;left: <?=  $hall['placesSets'][$i]['graphic_display']['left'] ?>px; top: <?=  $hall['placesSets'][$i]['graphic_display']['top'] ?>px;">
                            ...
                            <?php endfor; ?>
                            <?php if ($j < count($hall['placesSets']) - 1) $j++; ?>
                        <?php endfor; ?>
